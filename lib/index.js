(function (factory, window) {
  ...
  function setRandomPos() {
    for (const [i, v] of markerList.entries()) {
      const label = v.getTooltip();
      const labelDom = label._container;
      const markerDom = v._icon;
      const markerPosition = getPosition(markerDom);
  ...
  function computePositionStep(t) {
    ...
    // calculate layout
    for (const v of markerList) {
      const container = v.getTooltip()._container;
      const { disp } = v;
      let p = getPosition(container);
      const d = scaleTo(
        normalize(disp),
      ...
      L.DomUtil.setTransform(container, p);
    }
  }
  ...
  function setEdgePosition() {
    ...
    for (const v of markerList) {
      const container = v.getTooltip()._container;
      const tooltip = getPosition(container);
      ...
      L.DomUtil.setTransform(container, tooltip);
    }
  }
